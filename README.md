...
Я посчитала, что эти два поля и представляют собой весь алгоритм, поэтому далее просто снабдила код комментариями. Кажется, что так будет более наглядно.  

P.S. В коде много тернарных операторов, что пришлось использовать, чтобы повысить процент code coverage.
